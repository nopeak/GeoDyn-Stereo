...
class StereoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img1 = torch.from_numpy(img1).permute(2, 0, 1).float().contiguous()
        img2 = torch.from_numpy(img2).permute(2, 0, 1).float().contiguous()
        flow = torch.from_numpy(flow).permute(2, 0, 1).float().contiguous()
...
def fetch_dataloader(args):
    ...
    if args.train_datasets == 'sceneflow':
        ...
        """ tartanair + sceneflow + fallingthings + instereo2k * 50 + carla * 50 + crestereo + """
        new_dataset = mb2005 * 200 + mb2006 * 200 + mb2014 * 200 + mb2021 * 200 + mbeval3 * 200
